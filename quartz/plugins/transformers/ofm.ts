							...
							(value: string, ...capture: string[]) => {
								...
								if (value.startsWith('!')) {
									...
									if (
										...
										// Some modifications to add image align
										const [alt, ...transforms] = _alt.split('|');
										let dataAlign = 'center';

										if (transforms.indexOf('left') !== -1) {
											dataAlign = 'left';
										}
										...
										if (!alt.trim()) {
											return {
												type: 'html',
												value: `<span class="img-container" style="justify-content: ${dataAlign === 'left' ? 'flex-start' : dataAlign === 'right' ? 'flex-end' : dataAlign}">
										...
										return {
											...
											value: `<span class="img-container" style="justify-content: ${dataAlign === 'left' ? 'flex-start' : dataAlign === 'right' ? 'flex-end' : dataAlign}">
												<img src="${url}" width="${width}" height="${height}" alt="${alt}" ></img>
												<span class="img-caption" style="text-align: ${dataAlign}">${alt}</span>
											</span>`,
										};
									} else if (
									...
									children: [
										{
											...
											value: useDefaultTitle
												? capitalize(
														i18n(cfg.locale).components.callout[
															calloutType as ValidCallout
														] ?? calloutType,
													)
												: titleContent + ' ',
										},
